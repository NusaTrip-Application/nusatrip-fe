...
function ReviewDetailModal({
  ...
  const getItineraryImage = (url?: string | null) => {
    if (!url) return "/images/hero/pantaikuta.jpg";
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="flex gap-4 p-4 bg-bg-soft-blue rounded-xl border border-border-default">
            <img
              ...
              alt={review.itinerary?.title || "Itinerary Dihapus"}
              className="w-20 h-16 object-cover rounded-lg flex-shrink-0"
              onError={(e) => {
                const target = e.target as HTMLImageElement;
                if (!target.src.includes("pantaikuta.jpg")) {
                  target.src = "/images/hero/pantaikuta.jpg";
                }
              }}
            />
            <div className="flex flex-col justify-center">
              <p className="text-[10px] font-bold text-text-muted uppercase tracking-wide mb-1">Nama Itinerary</p>
              <p className="font-bold text-text-heading leading-snug">{review.itinerary?.title || "Itinerary Dihapus"}</p>
              <span className={`mt-1.5 text-[10px] font-bold px-2 py-0.5 rounded inline-block w-fit uppercase ${review.itinerary?.visibilityStatus === "PUBLISHED"
                  ? "bg-success/10 text-success"
                  : "bg-border-default/50 text-text-muted"
                }`}>
                {review.itinerary?.visibilityStatus || "TIDAK DIKETAHUI"}
              </span>
            </div>
          </div>
          ...
          <div className="p-4 bg-bg-soft-blue rounded-xl border border-border-default">
            ...
            <p className="font-bold text-text-heading">{review.reviewerUser?.fullName || review.reviewer?.fullName || review.user?.fullName || "User Tidak Diketahui"}</p>
          </div>
      ...
      <div className="relative bg-bg-surface rounded-2xl shadow-2xl w-full max-w-md p-6">
        <div className="flex flex-col items-center text-center gap-3 mb-6">
          ...
          <p className="text-sm text-text-body">
            ...
            <span className="font-bold text-text-heading">&quot;{review.reviewerUser?.fullName || review.reviewer?.fullName || review.user?.fullName || "User Tidak Diketahui"}&quot;</span>?
                ...
                <div key={review.reviewId} className="px-6 py-4 hover:bg-bg-hover transition-colors">
                  <div className="grid grid-cols-12 gap-4 items-center">
                    ...
                    <div className="col-span-3 flex items-center gap-3">
                      ...
                      />
                      <span className="font-semibold text-text-heading text-sm line-clamp-2">{review.itinerary?.title || "Itinerary Dihapus"}</span>
                    </div>

                    {/* Reviewer */}
                    <div className="col-span-2 text-sm text-text-body font-medium">
                      {review.reviewerUser?.fullName || review.reviewer?.fullName || review.user?.fullName || "User Tidak Diketahui"}
                    </div>
              ...
              <div className="flex items-center gap-1">
                ...
                {getPageNumbers().map((pageNum) => (
                  <button
                    ...
                    className={`w-8 h-8 rounded-lg text-sm font-bold transition ${page === pageNum
